...
func change_style(style_name := "", is_base_style := true, bubble: bool = false) -> Node:
    return load_style(style_name, null, is_base_style, true, bubble)
...
func load_style(style_name := "", parent: Node = null, is_base_style := true, state_reload := false, bubble: bool = false) -> Node:
    ...
    var new_layout := create_layout(style, parent, bubble)
    if state_reload:
...
func create_layout(style: DialogicStyle, parent: Node = null, bubble: bool = false) -> DialogicLayoutBase:
    # Load base scene
    var base_scene: DialogicLayoutBase
    var base_layer_info := style.get_layer_inherited_info("")
    if bubble:
        base_scene = load("res://addons/dialogic/Modules/DefaultLayoutParts/Base_TextBubble/text_bubble_base.tscn").instantiate()
    else:
